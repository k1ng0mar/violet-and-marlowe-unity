...
public class CarryItem : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (rb != null)
        {
            // Store the post-Awake carry-safe state (kinematic, no gravity) so Drop
            // restores this instead of the prefab's raw settings. Design intent:
            // the sack stays where dropped — it never simulates physics.
    ...
    public void PickUp(Transform carrier)
    {
        ...
        // Notify the heist state machine that loot is secured — without this,
        // the state never leaves VAULT_REACHED and extraction can never complete.
        var heist = FindObjectOfType<HeistManager>();
        if (heist != null)
            heist.OnLootSecured(this);
    }
    ...
    public void Drop()
    {
        ...
        if (rb != null)
        {
            // Restore carry-safe physics (kinematic, no gravity). Awake forces this
            // state per the no-physics design — the sack stays where it lands.
